...
const useYDoc = function (doc_id) {
  ...
  function updatePresence(newState) {
    awarenessRef.current.setLocalStateField && awarenessRef.current.setLocalStateField('presence', newState);
    awarenessRef.current.getStates && setTimeout(() => {
      console.log('newState: ', newState);
      console.log('updatePresence called: ', awarenessRef.current.getStates());
    }, 150);
  }
  ...
  useEffect(() => {
    ...
    // Throttled console output that will only output every 1000ms
    const throttledAwareStateUpdate = debounce(30, (newState) => {
      setAwarenessState((state) => ({
        ...state,
        ...newState,
      }));
